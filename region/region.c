...
void region_init(struct region_ctx *ctx, enum lorawan_region region)
{
	...
	case LORAWAN_REGION_RU864:
		...
		break;
	default:
		LOG_ERR("Unsupported region: %d, falling back to EU868", region);
		region_setup_eu868(ctx);
		break;
	}
}
			  ...
			  uint8_t offset)
{
	...
	/* Clamp to valid range — some regions (e.g. US915) return
	 * values >= num_dr meaning "RX1 not available"
	 */
	if (dr >= ctx->num_dr) {
		dr = ctx->num_dr - 1;
	}
			       ...
			       uint8_t tx_ch_idx)
{
	...
	default:
		/* EU868, KR920, IN865, AS923, CN779, RU864: RX1 = TX freq */
