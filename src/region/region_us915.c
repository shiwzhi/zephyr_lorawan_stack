 ...
 *
 * LoRaWAN US915 region implementation.
...
static const struct region_dr dr_table[US915_NUM_DR] = {
...
static const uint16_t dr_max_toa_ms[US915_NUM_DR] = {
	400, 250, 150, 100, 60, 100, 100, 100,
	30, 20, 20, 20, 20, 20,
};

static const uint8_t max_payload[US915_NUM_DR] = {
	19, 61, 133, 250, 250, 0, 0, 0,
	61, 137, 250, 250, 250, 250,
};

static const uint8_t rx1_dr_map[US915_NUM_DR][US915_NUM_DR] = {
...
void region_init(struct region_ctx *ctx, enum lorawan_region region)
{
	...
	ctx->num_channels = US915_NUM_UP_CH;
	for (uint8_t i = 0; i < US915_NUM_UP_CH; i++) {
		ctx->channels[i].freq_hz = freqs[i];
		ctx->channels[i].min_dr = 0;
		ctx->channels[i].max_dr = (i < US915_NUM_125K_CH) ? 4 : 9;
		ctx->channels[i].enabled = true;
	}
	ctx->num_dr = US915_NUM_DR;
	for (uint8_t i = 0; i < US915_NUM_DR; i++) {
...
{
	ARG_UNUSED(ctx);
	if (tx_dr >= US915_NUM_DR) tx_dr = US915_NUM_DR - 1;
	if (offset >= US915_NUM_DR) offset = US915_NUM_DR - 1;
...
{
	ARG_UNUSED(ctx);
	if (dr >= US915_NUM_DR) return -EINVAL;
...
{
	ARG_UNUSED(ctx);
	if (dr >= US915_NUM_DR) return 3000;
