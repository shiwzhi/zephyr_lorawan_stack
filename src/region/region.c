...
#endif
#ifdef CONFIG_LORAWAN_CUSTOM_REGION_EU433
...
static void region_setup_au915(struct region_ctx *ctx)
{
	...
	ctx->default_join_dr = 2;  /* DR2 for dwell-time compliance */
	ctx->beacon_freq = 923300000U;
}
#endif
...
#ifdef CONFIG_LORAWAN_CUSTOM_REGION_EU433
static void region_setup_eu433(struct region_ctx *ctx)
{
	ctx->region = LORAWAN_REGION_EU433;
	init_channels(ctx, EU433_NUM_CH, eu433_ch_freq, 0, EU433_NUM_DR - 1);
	init_dr_table(ctx, EU433_NUM_DR, eu433_dr_table);
...
#ifdef CONFIG_LORAWAN_CUSTOM_REGION_EU433
	case LORAWAN_REGION_EU433:
		dr = eu433_get_rx1_dr(tx_dr, offset);
		break;
#endif
	default:
		return 0;
...
#ifdef CONFIG_LORAWAN_CUSTOM_REGION_EU433
	case LORAWAN_REGION_EU433:
		return eu433_get_max_payload(dr);
#endif
	default:
...
uint32_t region_get_beacon_freq(uint32_t beacon_time)
{
	...
	return 869525000U;  /* Default; per-region value set in region_ctx.beacon_freq */
}
...
#ifdef CONFIG_LORAWAN_CUSTOM_REGION_EU433
	case LORAWAN_REGION_EU433:
		return eu433_get_rx_window_timeout_ms(dr);
#endif
	default:
		if (dr >= LORAWAN_MAX_DR) {
			return 3000;
...
#ifdef CONFIG_LORAWAN_CUSTOM_REGION_EU433
	case LORAWAN_REGION_EU433:
		return eu433_apply_cflist(ctx, cflist);
#endif
	default:
		return -EINVAL;
	}
